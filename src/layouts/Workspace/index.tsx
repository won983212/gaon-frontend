...
import { createChannel, createGroup, createWorkspace, deleteChannel, deleteGroup, deleteWorkspace, updateWorkspace, useChannelsSWR, useWorkspacesSWR, useWorkspaceSWR } from '@/api/workspace';
...
function Workspace({ children }: WorkspaceProps) {
    ...
    const [showWorkspaceUpdateDialog, setShowWorkspaceUpdateDialog] = useState(false);
    const [showChannelCreateDialog, setChannelShowCreateDialog] = useState(false);
    const [showWorkspaceCreateDialog, setShowWorkspaceCreateDialog] = useState(false);
    const [showChannelUpdateDialog, setShowChannelUpdateDialog] = useState(false);
    ...
    const onShowChannelContextMenu = (e: React.MouseEvent, groupId: number, channelId: number) => {
        e.preventDefault();
        ...
        (action: Action) => {
            ...
            if (action === 'yes') {
                ...
                switch (currentContextMenuInfo.type) {
                    case 'workspace':
                        if (currentContextMenuInfo.userId && currentContextMenuInfo.workspaceId) {
                            deleteWorkspace(userData.id, currentContextMenuInfo.workspaceId, identifier.token);
                        }
                        break;
                    case 'channel':
                        if (currentContextMenuInfo.userId && currentContextMenuInfo.channelId && currentContextMenuInfo.groupId)
                        deleteChannel(currentContextMenuInfo.userId, currentContextMenuInfo.channelId, currentContextMenuInfo.groupId, identifier.token);
                        break;
                    case 'group':
                        if (currentContextMenuInfo.groupId && currentContextMenuInfo.userId)
                        deleteGroup(currentContextMenuInfo.groupId, currentContextMenuInfo.userId, identifier.token);
                        break;
                }
            ...
            navigate(`/workspace/${workspaceId}/channel`);
                ...
                alert("작업에 실패 했습니다. 인터넷 연결이 불안정하거나 서버에서 요청한 명령을 수행하지 못했습니다.");
            }

        }, [navigate, workspaceId]
    );
    //얘는 따로따로
    const onCloseUpdateDialog = useCallback((action: Action) => {
        if (!userData) return;
        if (action === 'yes') {
            switch (currentContextMenuInfo.type) {
                case 'workspace':
                    let new_name = (document.getElementById('workspace-update-name') as HTMLInputElement)?.value;
                    updateWorkspace(userData?.id, workspaceId, identifier.token, new_name ?? "New workspace"); //에러 처리 필요
                    navigate(`/workspace/0/channel`);
            }
    ...
    const onCloseCreateDialog = useCallback((action: Action) => {
        if (!userData) return;
        if (action === 'yes') {
            switch (currentContextMenuInfo.type) {
                case 'workspace':
                    createWorkspace(userData?.id, "New workspace", identifier.token);
                    break;
                case 'channel':
                    if (currentContextMenuInfo.groupId)
                    createChannel(currentContextMenuInfo.groupId, userData?.id, "New group", "chatting", identifier.token);
                    break;
                case 'group':
                    if (currentContextMenuInfo.workspaceId)
                    createGroup("New group", currentContextMenuInfo.workspaceId, userData?.id, userData?.id, identifier.token);
                    break;
            }
        ...
        <div>
            <WorkspaceWrapper>
                <Channels>
                    <WorkspaceName onContextMenu={onShowWorkspaceContextMenu}>
                        <WorkspaceProfileWrapper>
                            <Profile
                                username={currentWorkspace?.name ?? "Select workspace"}
                                avatarUrl={gravatar.url(currentWorkspace?.name ?? "Select", {
                                    s: '36px',
                                    d: 'retro'
                                })}
                                rectAvatar
            ...
            <div>
                ...
                <ContextMenu.Menu id="workspace-context-menu">
                    <ContextMenu.Item onClick={()=>{setShowWorkspaceCreateDialog(true)}}>New workspace</ContextMenu.Item>
                    { workspaceId != 0 && (
                    <>
                        <ContextMenu.Item onClick={({props})=>{setShowWorkspaceUpdateDialog(true)}}>Update</ContextMenu.Item>
                        <ContextMenu.Item onClick={({props})=>{setShowDeleteDialog(true)}}>Delete</ContextMenu.Item>
                    </>
                    )}
                </ContextMenu.Menu>
                <ContextMenu.Menu id="channel-context-menu">
                <ContextMenu.Item onClick={()=>{}}>New channel</ContextMenu.Item>
                <ContextMenu.Item onClick={({props})=>{}}>Update</ContextMenu.Item>
                <ContextMenu.Item onClick={({props})=>{}}>Delete</ContextMenu.Item>
                </ContextMenu.Menu>
                <ContextMenu.Menu id="group-context-menu">
                    <ContextMenu.Item onClick={()=>{}}>New group</ContextMenu.Item>
                    <ContextMenu.Item onClick={({props})=>{setShowDeleteDialog(true)}}>Update</ContextMenu.Item>
                    <ContextMenu.Item onClick={({props})=>{setShowDeleteDialog(true)}}>Delete</ContextMenu.Item>
                </ContextMenu.Menu>
                <ContextMenu.Menu id="user-context-menu">
                    <ContextMenu.Item onClick={({props})=>{}}>Kick</ContextMenu.Item>
                    <ContextMenu.Item onClick={({props})=>{}}>Ban</ContextMenu.Item>
                </ContextMenu.Menu>                
            </div>
            <div>
                {showDeleteDialog && (
                    <Modal isOpen={true} onAction={(action: Action) => onCloseDeleteDialog(action)} buttons='yesno'>
                        정말로 삭제 하시겠습니까?
                    </Modal>
                )}
                {showWorkspaceCreateDialog && (
                    <Modal isOpen={true} onAction={onCloseCreateDialog} buttons='yesno'>
                        정말로 생성 하시겠습니까?
                    </Modal>
                )}
                {showWorkspaceUpdateDialog && (
                    <Modal isOpen={true} onAction={onCloseUpdateDialog} buttons='yesno'>
                            <Form>
